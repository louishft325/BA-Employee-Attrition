...
model_logistic <- "placeholder"

# Train kNN model
model_knn <- "placeholder"

# Train Decision Tree model with hyperparameter tuning
set.seed(123)
tune_grid_tree <- expand.grid(
cp = seq(0.01, 0.1, by = 0.01) # Control overfitting by tuning the complexity parameter
)
...
# Display the best parameters for the tree model
cat("Best parameters for Decision Tree model:\n")
print(model_tree$bestTune)

# Train Random Forest model
model_rf <- "placeholder"

# Train Gradient Boosting model
model_gbm <- "placeholder"

# Train Naive Bayes model
model_nb <- "placeholder"

# Evaluate each model on the test set and save the model
evaluate_model <- function(model, data) {
if (is.character(model)) {
return(list(confusion = NULL, auc = NULL, model = NULL))
}
...
auc_values <- sapply(results, function(res) if (!is.null(res$auc)) res$auc else NA_real_)
