...
# )
# 
# # Hyperparameter selection for Logistic Regression model
# tune_grid_logistic <- expand.grid()
# 
...
# metric = "ROC")
# 
# # Hyperparameter selection for kNN model
# tune_grid_knn <- expand.grid(k = seq(3, 21, by = 2))
# 
# # Train kNN model
...
# metric = "ROC")
# 
# # Hyperparameter selection for Decision Tree model
# tune_grid_tree <- expand.grid(cp = seq(0.01, 0.1, by = 0.01))
# 
...
# metric = "ROC")
# 
# # Hyperparameter selection for Random Forest model, added 'base::ceil' function to use the correct ceil in the global environment
# # tune_grid_rf <- expand.grid(mtry = base::ceil(sqrt(ncol(train_balanced)-1)))
# tune_grid_rf <- expand.grid(n.trees = c(50, 100, 150),
...
# metric = "ROC")
# 
# # Hyperparameter selection for Gradient Boosting model
# tune_grid_gbm <- expand.grid(n.trees = c(50, 100, 150), interaction.depth = c(1, 3, 5), shrinkage = 0.1)
# 
...
# }
# 
# # Hyperparameter selection for Naive Bayes model
# tune_grid_nb <- expand.grid(fL=c(0, 1), usekernel=c(TRUE, FALSE), adjust=seq(0.5, 2, length.out=4))
# 
...
# Disable parallel processing within rfe()
...
return(list(best_model = best_model, best_features = best_feature_size, auc = max_auc))
}
...
cat(paste("Naive Bayes AUC:", naive_bayes_results$auc, "with", naive_bayes_results$best_features, "features\n"))
cat(paste("Gradient Boosting AUC:", gradient_boosting_results$auc, "with", gradient_boosting_results$best_features, "features\n"))
cat(paste("Random Forest AUC:", random_forest_results$auc, "with", random_forest_results$best_features, "features\n"))

```
```{r}

# Hyperparameter Tuning for the Best GBR Model

# Extract best features and number from the best GBR model
best_feature_names <- names(gradient_boosting_results$best_model$importance)[1:gradient_boosting_results$best_features]
best_feature_count <- gradient_boosting_results$best_features

# Create formula with the best features
formula_best_features <- reformulate(termlabels = best_feature_names, response = "attrition")

# Define the hyperparameter grid for GBR
...
set.seed(123)
tuned_best_model <- train(
formula_best_features,
data = train_balanced,
method = "gbm",
trControl = train_control,
tuneGrid = gbm_grid,
...
cat("Hyperparameter Tuning for Best GBR Model:\n")
print(tuned_best_model)
print(tuned_best_model$bestTune)
